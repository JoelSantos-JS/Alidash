...
export function BetForm({ onSave, betToEdit, onCancel }: BetFormProps) {
  const form = useForm<z.infer<typeof betSchema>>({
    ...
    defaultValues: betToEdit ? (betToEdit.type === 'single' ? {
        ...
        sport: betToEdit.sport || "Futebol",
        event: betToEdit.event || "",
        date: new Date(betToEdit.date),
        status: betToEdit.status || 'pending',
        ...
        subBets: betToEdit.subBets || [],
         totalStake: betToEdit.totalStake || 0,
         guaranteedProfit: betToEdit.guaranteedProfit || 0,
         profitPercentage: betToEdit.profitPercentage || 0,
    }) : {
        type: 'single',
        sport: "Futebol",
        event: undefined,
