...
import { Wand2, PiggyBank, Flag, ListChecks, Info, Loader2 } from 'lucide-react';
...
export function DreamCard({ dream, plan, isPlanning = false, onPlan }: DreamCardProps) {
    ...
    <Card className="flex flex-col h-full overflow-hidden">
      ...
      <CardContent className="p-6 flex-1">
        ...
        {plan && (
            <div className='mt-6 space-y-4'>
                <div>
                    <h4 className='font-semibold flex items-center gap-2 mb-2'><PiggyBank className="w-5 h-5 text-primary"/> Custos Estimados</h4>
                 ...
                 <div>
                    <h4 className='font-semibold flex items-center gap-2 mb-2'><ListChecks className="w-5 h-5 text-primary"/> Plano de Ação</h4>
                 ...
                 <div>
                    <h4 className='font-semibold flex items-center gap-2 mb-2'><Info className="w-5 h-5 text-primary"/> Dicas Importantes</h4>
      ...
      <CardFooter className="p-6 bg-secondary/30">
        <Button className="w-full" onClick={onPlan} disabled={isPlanning}>
            {isPlanning ? (
                <><Loader2 className="mr-2 animate-spin"/> Gerando plano com IA...</>
            ) : (
                <><Wand2 className="mr-2"/> {plan ? 'Gerar Novo Plano' : 'Planejar com IA'}</>
            )}
        </Button>
      </CardFooter>
    </Card>
